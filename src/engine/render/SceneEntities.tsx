...
const HUDPlaneRenderer = ({ entity, isGameView }: { entity: Entity; isGameView: boolean }) => {
  ...
  const handleBeforeRender = (_renderer: any, _scene: any, currentCamera: THREE.Camera) => {
    const shouldStick = isPlaying || isGameView;
    const mesh = meshRef.current;
    if (mesh) {
      if (shouldStick) {
...
function PerspectiveCameraWrapper({ entity, camera, isGameView, isStandalone }: { entity: Entity; camera: any; isGameView: boolean; isStandalone: boolean }) {
  ...
  const xrIsFirstFrame = useRef(true);
  const xrInitialHeadsetHeight = useRef<number | null>(null);
  ...
  useFrame((state, delta) => {
    ...
    // Quando false, a câmera VR NÃO é controlada pelo headset — os Scripts do jogo
    // continuam controlando câmera e transform normalmente (igual ao modo Screen).
    // Ideal para câmeras de 3ª pessoa, FPV de drone, câmera orbital, etc.
    const useGyroscope = camera?.useGyroscope !== false; // undefined ou true → gyroscope ativo
    ...
    if (state.gl.xr.isPresenting && !simulatedAR) {
      ...
      if (camera.isMain) {
        ...
        if (xrGroup) {
          ...
          let eRotY = entity.components.Transform?.rotation?.[1] || 0;
          ...
          if (rb) {
            try {
              ...
              xrQuatTemp.current.set(rot.x, rot.y, rot.z, rot.w);
              xrEulerTemp.current.setFromQuaternion(xrQuatTemp.current);
              eRotY = xrEulerTemp.current.y * 180 / Math.PI;
            } catch (_e) {}
          }
          ...
          if (xrCamObj) {
            const h = xrCamObj.position.y;
            localHPos.copy(xrCamObj.position);
            if (xrInitialHeadsetHeight.current === null && h >= 0.5) {
              xrInitialHeadsetHeight.current = h;
            }
          }
          ...
          if (xrIsFirstFrame.current) {
            xrLastHLocalWorld.current.copy(hLocalWorld);
            xrIsFirstFrame.current = false;
          } else {
            const diff = hLocalWorld.clone().sub(xrLastHLocalWorld.current);
            if (diff.lengthSq() > 0.00001) {
              if (rb) {
                const t = rb.translation();
                rb.setTranslation({ x: t.x + diff.x, y: t.y, z: t.z + diff.z }, true);
                ePos = [t.x + diff.x, t.y, t.z + diff.z];
              } else {
                const cur = entity.components.Transform?.position || [0, 0, 0];
                const next: [number, number, number] = [cur[0] + diff.x, cur[1], cur[2] + diff.z];
                updateComponentRef.current(entity.id, 'Transform', { position: next });
                ePos = next;
              }
              xrLastHLocalWorld.current.copy(hLocalWorld);
            }
          ...
          const targetY = xrInitialHeadsetHeight.current !== null
            ? ePos[1] + xrCurrentHeight.current - xrInitialHeadsetHeight.current
            : ePos[1] + xrCurrentHeight.current;

          xrGroup.position.set(ePos[0] + offset[0] - hLocalWorld.x, targetY, ePos[2] + offset[2] - hLocalWorld.z);
          xrGroup.rotation.set(0, eRotY * Math.PI / 180, 0);
        }
      }
    ...
    return (
      <Suspense fallback={null}>
        <GLTFMesh entity={entity}>
          ...
          {entity.childrenIds && entity.childrenIds.map(childId => (
            <EntityMesh key={childId} id={childId} />
          ))}
        </GLTFMesh>
      </Suspense>
    );
...
function XRSync() {
  ...
  useEffect(() => {
    xrOriginGroupRef.current = groupRef.current;
    return () => { xrOriginGroupRef.current = null; };
  }, []);
...
export function SceneEntities() {
  ...
  return (
    <>
      ...
      {isStandalone && <XRSync />}
      {rootEntityIds.map((id: string) => (
        <EntityMesh key={id} id={id} />
      ))}
    </>
  );
}
