...
export function GLTFModelRenderer({ entity }: { entity: Entity }) {
  ...
  const { ref: mixerRef, actions, names } = useAnimations(gltf.animations);
  ...
  useEffect(() => {
    if (mixerRef && clonedScene) {
      mixerRef.current = clonedScene;
    }
  }, [mixerRef, clonedScene]);
  ...
  useEffect(() => {
    if (animator && names && names.length > 0) {
      ...
      if (hasChanged) {
        useEditorStore.getState().updateComponent(entity.id, 'Animator', {
  ...
  }, [clonedScene, model.overrideMaterial, model.color, model.roughness, model.metalness, model.textureUrl, model.normalMapUrl, model.normalScale, isStandalone]);
  ...
  if (isPlaying && rigidBody && rigidBody.collider === 'trimesh') {
    return (
